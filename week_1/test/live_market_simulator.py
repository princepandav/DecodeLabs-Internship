...
class LiveMarketSimulator:
    ...
    def _bootstrap_system(self):
        """Initializes components and validates production file structures."""
        # 1. Load the trained machine learning brain
        ...
        # =================================================================
        # FIX: CALCULATE RUNTIME COLUMNS EXPECTED BY XGBOOST
        # =================================================================
        ...
        # Mirror the exact math used during the training setup
    ...
    def start_live_simulation(self, start_str, end_str, confidence_threshold=0.55):
        """Streams market data chronologically, executing real-time predictions and trade tracking."""
        try:
        ...
        # Filter the dataset for the simulated live trading window
        sim_data = self.df.loc[(self.df.index >= start_dt) & (self.df.index <= end_dt)]
        
        if sim_data.empty:
            print(f"⚠️ No market data windows discovered between {start_str} and {end_str}")
            return
        ...
        for current_time, row in sim_data.iterrows():
            ...
            # Real-time Trade Management Layer (Update existing open positions)
            ...
            # Render scrolling live terminal log
        ...
        # =================================================================
        # METRIC COMPILATION SUMMARY REPORT
        # =================================================================
        print("\n" + "="*60)
        ...
        if len(closed_trades) > 0:
            ...
            # Calculate dynamic capital tracking metrics based on standard ₹3,000 risk units
            monetary_pnl = net_r * 3000
            print(f"Net Financial Performance     : {'+₹' if monetary_pnl >=0 else '-₹'}{abs(monetary_pnl):,}")
        print("="*60)

if __name__ == "__main__":
    # Configure production target dependencies 
    ...
    try:
        ...
        while True:
            ...
            # Execute calculation arrays across the filtered parameters
